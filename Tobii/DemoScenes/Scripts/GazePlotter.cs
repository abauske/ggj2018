...
public class GazePlotter : MonoBehaviour
{
	...
	public Transform eyeTrackerSelect;

	//private GazePoint sharedGaze;
	...
	void Update()
	{
	//	sharedGaze = TobiiAPI.GetGazePoint ();
	...
	public void SelectObjectsInGaze(GazePoint gazepoint){
		Vector3 gazePointInWorld = ProjectToPlaneInWorld(gazepoint);
		Debug.Log(Physics2D.OverlapCircle (Smoothify (gazePointInWorld), 1)); 
		if (Physics2D.OverlapCircle (Smoothify (gazePointInWorld), 1, 1 << 8) != null) {
			Transform nearestObject = Physics2D.OverlapCircle (Smoothify (gazePointInWorld), 1, 1 << 8).transform;
		


		
			
			eyeTrackerSelect.position = nearestObject.position + Vector3.back;
			eyeTrackerSelect.GetComponent<LightDimmer> ().whatToDo = "dimmUp";
		} else {
			eyeTrackerSelect.GetComponent<Light> ().intensity = 0;
		}
		}
	}
